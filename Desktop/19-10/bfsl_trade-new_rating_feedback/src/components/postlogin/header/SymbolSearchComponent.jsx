...
import { HEADER, WATCHLIST } from '../../../config/ServiceURLs'
...
import {
    ...
    storeToastMsgProps,
    storeWatchlistDialogDetails
} from '../../../state/actions/Actions'

import {
    isValidSearchInput, getWatchlistBaseUrl, getMarketDataBaseURL,
    getDispSymbolName, getFormatedDate, cacheSearch, convertToLowerCase, applyPaint, AF_EventTriggered
} from '../../../common/CommonMethods';

import {
    ASSET_MENU_ARRAY, SEGMENT_ARRAY, SCREENS, ENABLE_CACHE_SEARCH, TEXT_ORIENTATION, STREAMING_MODULES,
    STREAMING_KEYS, LOCAL_STORAGE, WATCHGROUP_MAX_LIMIT, AF_EVENT_TYPES, AF_EVENT_NAMES,
} from '../../../common/Constants';
import { CloseIcon, SearchIcon } from '../../common/FontIcons'
import { getItemFromSessionStorage } from '../../../common/LocalStorage';
import { getEditableWatchgroups, gotoAlert, gotoQuote, gotoTrade } from '../../../common/Bridge';
...
function SymbolSearchComponent(props) {
    ...
    useEffect(() => {
        ...
        if (watchGroups.length) {
            ...
            if (!props.location.pathname.includes(SCREENS.DASHBOARD))
                getWatchGroup()
    ...
    }, [props.selectedWatchgroupResp, props.selectedWatchgroup, selectedSym])
    ...
    function symbolSearch(searchString) {
        ...
        MsfFetch.placeRequest(
            getMarketDataBaseURL() + HEADER.SYMBOL_SEARCH,
            request,
    ...
    function successCBSearchSymbols(response) {
        ...
        AF_EventTriggered(AF_EVENT_NAMES.WATCHLIST, AF_EVENT_TYPES.SEARCH)
    }
    ...
    function errorCBSearchSymbols(error) {
        ...
        AF_EventTriggered(AF_EVENT_NAMES.WATCHLIST, error.message)
    }
    ...
    function onClickAddSymBtn(symObj, symbolAction) {
        ...
        if (symbolAction == "add") {
            ...
            if (props.newWatchListName && props.isNewWatchList) {
                ...
                MsfFetch.placeRequest(
                    getWatchlistBaseUrl() + WATCHLIST.ADD_WATCHGROUP,
                    request,
                ...
                MsfFetch.placeRequest(
                    getWatchlistBaseUrl() + WATCHLIST.ADD_SYMBOLS,
                    request,
                    successCBAddSymbolsOnCheck,
                    errorCBAddSymbolsOnCheck
                )
    ...
    function successCBAddSymbolsOnCheck(response) {
        ...
        if (props.newWatchListName && props.isNewWatchList) {
            ...
            console.log("props.newWatchListName", props.newWatchListName)
            ...
            console.log("else props.newWatchListName")
            props.storeToastMsgProps({
                show: true,
                message: response.infoMsg
            })
        ...
        AF_EventTriggered(AF_EVENT_NAMES.WATCHLIST, AF_EVENT_TYPES.WATCHLIST_SUCCESS_COUNT)
    }
    ...
    function errorCBAddSymbolsOnCheck(error) {
        ...
        AF_EventTriggered(AF_EVENT_NAMES.WATCHLIST, AF_EVENT_TYPES.WATCHLIST_FAILURE_COUNT)
    }
    ...
    function onClickBuy_Sell(symData, tradeType) {
        ...
        if (props.history && props.history.location.pathname !== SCREENS.DASHBOARD) {
    ...
    function getSelectedAddFlag(symObj) {
        ...
        console.log("watchlist-symbol", watchListSymbols)
        // console.log("symbolList",symbolList)

        let selected = false
        watchListSymbols.map((item) => {
            // console.log("watchlist item.sym.id",item.sym.id)
    ...
    // function getShowOptionFlag(flag) {
    //     setIsSelectOptionVisible(flag)
    // }
                        ...
                        ${isSelectOptionVisible ? 'reduceOp' : ''}`}>
                            {
                                (symbolList.length && showRecentSearch) ?
                                    <div className="recent-search-title">
                                        <span>
                                            <LangText module="SYMBOL_SEARCH" name="RESENT_SEARCH" />
                                        </span>
                            ...
                            />
                            {
                                (!symbolList.length && !searchValue.length) ?
                                    <div className="flex-center infoMsg">
                                        <LangText module="SYMBOL_SEARCH" name="NO_RESENT_SEARCH" />
                                    </div>
                                    : null
                            }
                        </div>
                                                ...
                                                <div className="action-div">
                                                    <div className="flex-center">
                                                        <span className="addToText">
                                                            <LangText module="SYMBOL_SEARCH" name="ADD_TO" />
                                                        </span>
                                                        {
                                                    ...
                                                    >
                                                        <LangText module="BUTTONS" name="ADD_SYMBOL" />
                                                    </button>
                                                </div>
                                            </div>
                                            : null
