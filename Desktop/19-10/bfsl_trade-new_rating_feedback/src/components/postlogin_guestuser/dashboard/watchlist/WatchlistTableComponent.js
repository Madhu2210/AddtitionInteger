...
import { HOLDINGS_GUEST, WATCHLIST } from '../../../../config/ServiceURLs'
...
import {
    ...
    getColorCode, getWatchlistBaseUrl, AF_EventTriggered, getGuestUserBaseURL, sortByString, 
    sortByInt, convertToUpperCase
...
import {
    gotoChartPopup, gotoQuote, gotoTrade, hideToaster,
    storeHoldingsResponseToStore,
    storeStreamingDatatoStore
} from '../../../../common/Bridge'
...
function WatchlistTableComponent(props) {
    ...
    function addHoldingsDataToList() {
        ...
        if (props.holdingsResp && props.holdingsResp.length) {
            symResponseData.map(function (item) {
                ...
                let hIndex = props.holdingsResp.findIndex((hItem) => {
                    return item.sym.excToken === hItem.sym.excToken
                })
                if (hIndex !== -1) {
                    item.netQty = props.holdingsResp[hIndex].qty
                }
                else
                    item.netQty = 0
                return item
    ...
    function showMoreDetails(index) {
        ...
        AF_EventTriggered(AF_EVENT_NAMES.WATCHLIST, AF_EVENT_TYPES.MORE)
    }
