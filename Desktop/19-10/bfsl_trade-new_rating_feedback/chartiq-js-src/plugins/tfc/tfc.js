...
CIQ.TFC = function (config) {
	...
	if (stx.translateUI) stx.translateUI(stx.container.ownerDocument);
...
CIQ.TFC.prototype.construct = function (config) {
	...
	var topNode = config.context
		...
		: config.stx.container.ownerDocument;
	...
	this.stx.container.ownerDocument.defaultView.addEventListener(
		"resize",
		(function (self) {
			return function () {
				self.refreshScrollWindows();
