...
CIQ.Account.Demo.prototype.placeOrder = function (tfc, order, cb) {
	...
	let sessionID = null;
	let authToken = null;
	let isEncryptionEnabled = null;
	let headers = null

	let data = (stxx && stxx.tfc && stxx.tfc.selectedSymbols) ? stxx.tfc.selectedSymbols : getQueryParams();

	data && data.encryptionKey ? ENCRYPTION_KEY = data.encryptionKey : ENCRYPTION_KEY = null;
	data && data.appID ? appID = data.appID : appID = null;
	data && data.sessionID ? sessionID = data.sessionID : sessionID = null
	data && data.authToken ? authToken = data.authToken : authToken = null
	data && data.isEncryptionEnabled ? isEncryptionEnabled = data.isEncryptionEnabled : null

	if (data.deviceType === 'mobile') {
		headers = {
			'Content-Type': 'application/json',
			'X-ENCRYPT': false,
			'X-JSESSIONID': sessionID,
	...
	let orderQty = order.quantity ? order.quantity : (order.qty ? order.qty : '')

	let apiRequestObj = {
		request: {
			data: {
				"sym": data.symbolObject,
				"prdType": order.pType,
				"ordType": order.tif,
				"limitPrice": order.tif === "LIMIT" && parseFloat(order.limit) ? (order.limit).toString() : parseFloat(order.stop) ? (order.stop).toString() : undefined,
				"isAmo": false,
				"qty": orderQty.toString(),
				"ordAction": order.action,
				"ordDuration": "DAY",
	...
	let fetchRequestObj = {
		method: 'POST',
		credentials: 'include',
		body: JSON.stringify(apiRequestObj),
		headers: headers
	}
	fetch(data.tradingURL, fetchRequestObj)
		.then(checkStatus)
		.then(getBody)
		.then(parseBody)
		.then(response => {
			...
			if (result.infoID === "0" && result.data) {
				...
				if (window.chatIq) {
					...
					if (data && data.platform === 'android')
						window.chatIq.postMessage(sendObj2);
					else
						window.chatIq.postMessage(sendObj2, '*');
				}
				...
				if (window.ReactNativeWebView) {
					let data = getQueryParams();
					if (data && data.platform === 'android')
						window.ReactNativeWebView.postMessage(sendObj2)
					else
						window.ReactNativeWebView.postMessage(sendObj2, '*')
				}
			} else if (result.infoID === "EGN0024") {
				cb(result.data);
			}
			else {
				cb(result);
