...
import {
    ...
    storeOrderPadDialogDetails, storeOrderpadSelectedSym, storeSquareoffNetPosition
...
import { gotoChartPopup, gotoOrderDialog, gotoQuote,
    gotoTrade } from '../../../../../common/Bridge';
import {
    ...
    TRADE_ORDER_TYPES, THEMES, AF_EVENT_NAMES, AF_EVENT_TYPES, ORDER_MODIFY_CHILD_TYPES, 
    TRADE_PRODUCT_TYPES, ORDER_MODIFY_BO_CO
...
function OrderBookTableComponent(props) {
    ...
    useEffect(() => {
        AF_EventTriggered(AF_EVENT_NAMES.ORDERBOOK , AF_EVENT_TYPES.ORDERBOOK_TAB,{"onLoad":"orderbook"})
    }, [])
    ...
    function onClickModifyOrder(dataItem, type) {
        ...
        if(dataItem.childType) {
            if(dataItem.prdType === TRADE_PRODUCT_TYPES.CO) {
                if(convertToUpperCase(dataItem.childType) === ORDER_MODIFY_CHILD_TYPES.MAIN) {
                    values[ORDERPAD_FIIELD_KEYS.PRODUCT_TYPE] = ORDER_MODIFY_BO_CO.CO_MAIN
                }
                else if(convertToUpperCase(dataItem.childType) === ORDER_MODIFY_CHILD_TYPES.SECOND) {
                    values[ORDERPAD_FIIELD_KEYS.PRODUCT_TYPE] = ORDER_MODIFY_BO_CO.CO_SECOND
                }
                // pass childType as 'second' after manipulating from FE eventhough MW is sending as 'third' errorneously due to UAT env issues from client side
                else if(convertToUpperCase(dataItem.childType) === ORDER_MODIFY_CHILD_TYPES.THIRD) {
                    values[ORDERPAD_FIIELD_KEYS.PRODUCT_TYPE] = ORDER_MODIFY_BO_CO.CO_SECOND
                    symObj.childType = "second"
                }
            }
            else if(dataItem.prdType === TRADE_PRODUCT_TYPES.BO) {
                if(convertToUpperCase(dataItem.childType) === ORDER_MODIFY_CHILD_TYPES.MAIN) {
                    values[ORDERPAD_FIIELD_KEYS.PRODUCT_TYPE] = ORDER_MODIFY_BO_CO.BO_MAIN
                }
                else if(convertToUpperCase(dataItem.childType) === ORDER_MODIFY_CHILD_TYPES.SECOND) {
                    values[ORDERPAD_FIIELD_KEYS.PRODUCT_TYPE] = ORDER_MODIFY_BO_CO.BO_SECOND
                }
                else if(convertToUpperCase(dataItem.childType) === ORDER_MODIFY_CHILD_TYPES.THIRD) {
                    // to handle ordertype erroneously coming as SL instead of SL-M for second leg orders, where MW will be sending childType as 'third' for both second and third legs
                    if(dataItem.ordType === TRADE_ORDER_TYPES.SL) {
                        values[ORDERPAD_FIIELD_KEYS.PRODUCT_TYPE] = ORDER_MODIFY_BO_CO.BO_SECOND_SL
                        symObj.childType = "second"
                    }
                    else 
                        values[ORDERPAD_FIIELD_KEYS.PRODUCT_TYPE] = ORDER_MODIFY_BO_CO.BO_THIRD
                }
            }
        }
                            ...
                            return (
                                <>
                                    ...
                                    ? "buy-clr" : isSellTradeAction(item.ordAction)
                                                        ...
                                                        }
                                                        {
                                                            item.isAmo ?
                                                                <span className="pTypeVal buy-clr">
                                                                    <LangText 
                                                                        module="ORDERBOOK_MSGS" name="AMO" /></span>
                                                                : null
                                                        }
                                                    </span>
                                                </div>
                                        ...
                                        <td>
                                            <span>
                                                {/* Show trigger price in price column for BO and CO second leg pending orders */}
                                                {convertToUpperCase(item.ordType) ===
                                                    TRADE_ORDER_TYPES.MARKET ? 
                                                    item.status === ORDER_STATUS.PENDING &&
                                                     item.prdType === TRADE_PRODUCT_TYPES.CO && item.childType && 
                                                 item.childType === "second" ? checkEmpty(item.triggerPrice) :
                                                        checkEmpty(item.avgPrice) :
                                                    item.status === ORDER_STATUS.PENDING && 
                                                    (item.prdType === TRADE_PRODUCT_TYPES.CO ||
                                                         item.prdType === TRADE_PRODUCT_TYPES.BO)  && item.childType &&
                                                     item.childType === "second" ? checkEmpty(item.triggerPrice) :
                                                        checkEmpty(item.limitPrice)}
                                            </span>
                                        </td>
                                        <td>
